...
float triX( vec2 uv ) {
   float s1 = sdBox(Rot(PI*1./4.) * abs(uv) - vec2(0.45, 0.45), vec2(0.25,0.25));
   float m1 = S(.008, 0., s1);
   float s2 = sdBox(Rot(PI*3./4.) * abs(uv) - vec2(-0.45, -0.45), vec2(0.25,0.25));
   float m2 = S(.008, 0., s2);
   return m1 + m2;
}

// cross beam
float crossBeam2( vec2 uv) {
    float s1 = sdBox(uv - vec2(0.0, 0.5), vec2(0.5, 0.25));
    float m1 = S(.008, 0., s1);
    float s2 = sdBox(uv - vec2(0.0, -0.5), vec2(0.5, 0.25));
    float m2 = S(.008, 0., s2);
    float mm = 1. - ( m1 + m2 );
    float s3 = triangles( uv * 2.);
    float m3 = S(.008, 0., s3);
    return  mm + m3 - max(mm, m3);  
}
...
vec3 chooseShape( float shapechoice, vec2 uv, vec3 col1, vec3 col2 ) {
  ...
  // Half Box
    ...
    float crb = crossBeam( uv );
    col += (1. - crb) * col1 + crb * col2;
  }
else if (shapechoice == 3.0) {
    float crb = crossBeam2( uv );
    col += (1. - crb) * col1 + crb * col2;
  }
 return col;
}
