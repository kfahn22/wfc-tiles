...
new p5(sa => {
    ...
    sa.setup = () => {
        ...
        if (parameters !== null) {
            ...
            ySlider.value(parameters.y);
        }
        radSlider.changed(sa.storeParameters);
        xSlider.changed(sa.storeParameters);
        ySlider.changed(sa.storeParameters);
        ...
        div3a.position(10, 10);
        radlabel = sa.createP('Radius');
        radlabel.parent(div3a);
        radlabel.style("color", "#555555");
        radSlider.style("width", "180px");
        radSlider.parent(div3a);

        let div4a = sa.createDiv();
        div4a.style('font-size', '16px');
        div4a.position(10, 70);
        xlabel = sa.createP('X offset');
        xlabel.parent(div4a);
        xlabel.style("color", "#555555");
        xSlider.parent(div4a);
        xSlider.style("width", "180px");

        let div5a = sa.createDiv();
        div5a.style('font-size', '16px');
        div5a.position(10, 135);
        ylabel = sa.createP('Y offset');
        ylabel.parent(div5a);
        ylabel.style("color", "#555555");
        ySlider.parent(div5a);
        ySlider.style("width", "180px");
    ...
    sa.draw = () => {
        ...
        let x = xSlider.value();
        let xOffset = sa.map(x, 0, 300, -1.5, 1.5);
        let y = ySlider.value();
        let yOffset = sa.map(y, 0, 300, -1.5, 1.5);
...
new p5(sb => {
    ...
    sb.draw = () => {
        ...
        let x = xSlider.value();
        let xOffset = sb.map(x, 0, 300, 1.5, -1.5);
        let y = ySlider.value();
        let yOffset = sb.map(y, 0, 300, -1.5, 1.5);
...
new p5(sc => {
    ...
    sc.draw = () => {
        ...
        let x = xSlider.value();
        let xOffset = sc.map(x, 0, 300, -1.5, 1.5);
        let y = ySlider.value();
        let yOffset = sc.map(y, 0, 300, 1.5, -1.5);
...
new p5(sd => {
    ...
    sd.draw = () => {
        ...
        let x = xSlider.value();
        let xOffset = sd.map(x, 0, 300, 1.5, -1.5);
        let y = ySlider.value();
        let yOffset = sd.map(y, 0, 300, 1.5, -1.5);
